...
import { useLengthData } from "../../hooks/categories/useLengthData";
import { removeVietnameseTones } from "../../utils/searchUtils";
import { IconArrowDown } from "../icon/ArrowDown";

const LengthSelect: React.FC<SelectProps<LengthData>> = ({
  value,
  defaultData,
  onChange,
  onChangeData,
  ...
  const handleChange = (value: number) => {
    onChange?.(value);
    const length = lengthData.find((item) => item.id === value);
    onChangeData?.(length);
  };
  ...
  return (
    <div className="flex flex-row">
      <Select
        ...
        loading={loading}
        showSearch
        placeholder={placeholder !== undefined ? placeholder : "Chọn độ dài"}
        onFocus={(e) => {
          setIsLockHook(false);
          onFocus?.(e);
        }}
      ...
      />
      <ManagerModal<LengthData>
        label="Danh sách độ dài"
        open={open}
